...
DEFAULT_SYMBOLS: str = "AUTO"        # رموز مفصولة بفواصل أو AUTO للاختيار التلقائي
DEFAULT_AUTO_SYMBOL_LIMIT: int = 15  # أقصى عدد يتم اختياره تلقائيًا عند استخدام AUTO
...
def _parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--symbols", default=DEFAULT_SYMBOLS,
                   help="قائمة رموز مفصولة بفواصل (USDT-M) أو AUTO للاختيار التلقائي")
    ...
    p.add_argument("--auto-limit", type=int, default=DEFAULT_AUTO_SYMBOL_LIMIT,
                   help="عدد الرموز التي يتم التقاطها تلقائيًا عند استخدام AUTO")
    args, _ = p.parse_known_args(argv)
    return args


def _auto_discover_strategy_symbols(limit: int = DEFAULT_AUTO_SYMBOL_LIMIT) -> List[str]:
    if limit <= 0:
        return []
    if ccxt is None:
        return []
    cfg = _CLISettings()
    cfg.max_scan = max(1, limit)
    try:
        candidates = _pick_symbols(cfg, max_symbols_hint=cfg.max_scan)
    except Exception as exc:
        print(f"[AUTO] تعذر اختيار الرموز تلقائيًا: {exc}", file=sys.stderr)
        return []
    selected: List[str] = []
...
def _main(argv: Optional[List[str]] = None) -> None:
    ...
    auto_requested = (not requested) or any(token.upper() == "AUTO" for token in requested)
    manual_symbols = [token for token in requested if token.upper() != "AUTO"]

    auto_symbols: List[str] = []
    if auto_requested:
        auto_limit = max(1, int(getattr(args, "auto_limit", DEFAULT_AUTO_SYMBOL_LIMIT) or DEFAULT_AUTO_SYMBOL_LIMIT))
        auto_symbols = _auto_discover_strategy_symbols(auto_limit)
        if auto_symbols:
            print("[AUTO] تم اختيار الرموز التالية تلقائيًا:", ", ".join(auto_symbols))
        else:
            print("[AUTO] تعذر الحصول على قائمة تلقائية، سيتم الاعتماد على المدخلات اليدوية إن وجدت.")

    symbols = manual_symbols + [sym for sym in auto_symbols if sym not in manual_symbols]
    if not symbols:
        fallback = "BTCUSDT"
        print(f"[AUTO] استخدام الرمز الافتراضي {fallback} لعدم توفر خيارات أخرى.")
        symbols = [fallback]
